...
const (
	apiURL = "https://api.ultodo.io"
)

// Backend is giving you the structure of the ultodo API backend.
type Backend struct {
...
// PerformRequest is performing a request to the ultodo API backend.
func (b *Backend) PerformRequest(method string, path string, data []byte) []byte {
	...
	req.Header.Set("x-ultodo-CLI-Version", VERSION)
...
// CanConnect is checking if ultodo can connect to the API backend.
...
func (b *Backend) WriteCreds(token string) {
	...
	if _, err := os.Stat(b.credsFolderPath()); os.IsNotExist(err) {
		if err := os.MkdirAll(b.credsFolderPath(), os.ModePerm); err != nil {
			fmt.Println("Could not create ~/.config/ultodo directory! Permissions issue?")
			os.Exit(1)
		}
	}

	if err := os.WriteFile(b.credsFilePath(), data, 0600); err != nil {
...
func (b *Backend) apiURL(path string) string {
	envAPIURL := os.Getenv("ultodo_API_URL")
...
func (b *Backend) credsFolderPath() string {
	home := UserHomeDir()
	return fmt.Sprintf("%s/.config/ultodo/", home)
}
...
func (b *Backend) loadCreds() string {
	data, err := os.ReadFile(b.credsFilePath())
	if err != nil {
		panic(err)
	}
