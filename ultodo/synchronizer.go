...
// ExecSyncInBackground is starting a new sync process with the ultodo API in the background.
func (s *Synchronizer) ExecSyncInBackground() {
	binary, lookErr := exec.LookPath("ultodo")
	if lookErr != nil {
		panic(lookErr)
	}
...
// Sync is synchronizing the todos with the ultodo API.
func (s *Synchronizer) Sync(todolist *TodoList, syncedList *SyncedList) {
	...
	if s.Backend.CanConnect() == false {
		s.println("Cannot connect to api.ultodo.io right now.")
		return
	}
...
// CheckAuth is checking the authentication status against the ultodo API.
func (s *Synchronizer) CheckAuth() {
	if s.Backend.CredsFileExists() == false {
		fmt.Println("It looks like you are not authenticated with ultodo.io.")
		return
	}

	if s.Backend.CanConnect() == false {
		fmt.Println("Cannot connect to api.ultodo.io right now.")
		return
	}
