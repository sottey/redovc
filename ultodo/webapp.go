...
// Run is starting the ultodo webapp.
func (w *Webapp) Run() {
...
func (w *Webapp) handleAuthResponse(writer http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("Authorization successful! Next, run `ultodo sync --setup` to sync a list.")
...
func (w *Webapp) frontendUrl() string {
	envFrontendURL := os.Getenv("ultodo_FRONTEND_URL")

	if envFrontendURL != "" {
		return envFrontendURL + "/todolist?cli_auth_completed=true"
	}

	return "https://app.ultodo.io/todolist?cli_auth_completed=true"
}
